...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, args, num_classes=1000,
        ...
        self.mlp = nn.Linear(512, args.width)
        self.bn1d = nn.BatchNorm1d(args.width)
        self.repr_network = nn.ModuleList()
        if args.identity:
            self.repr_network.append(nn.Identity(args.width))
        elif args.quantum:
            self.repr_network.append(QNet(n_qubits=args.width, encoding=args.encoding, ansatz_type=args.q_ansatz,
            ...
            for i in range(args.layers):
                self.repr_network.append(nn.Linear(args.width, args.width, bias=True))
                self.repr_network.append(nn.LeakyReLU())

        self.fc = nn.Linear(args.width * block.expansion, num_classes)
    ...
    def forward(self, x):
        ...
        if self.args.batchnorm:
            x = self.bn1d(x)
        x = torch.sigmoid(x / self.args.sigtemp)
        x = x * np.pi
        for f in self.repr_network:
            x = f(x)
        x = self.fc(x)
        return x
